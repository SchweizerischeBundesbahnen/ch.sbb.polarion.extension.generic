...
- **Maven profile and property names follow one scheme.** A profile is `<group>-<what it does>`, a property is `<group>.<subject>.<action>`, and both carry the same group, so `-Dui.tests.withoutDocker` activates `ui-tests-run-without-docker`. Groups in use: `ui-` (the React app build and its tests), `release-` (signing and publishing), `local-` (developer machine). Do not name a profile after the tool it happens to call. Renaming one breaks every downstream workflow that passes it, so the deprecated property aliases at the end of the properties block (`vite.*`, `jsTest*`, `generic.app.ui.webapp`) stay until the next major: an unknown `${...}` resolves to its own literal text instead of failing. `gpg-sign`, `central-publishing` and `-DskipJsTests` are the three names downstream CI still relies on; renaming those is tracked separately.
